...
import { iGIFObj, iPixel } from './Helpers';
import { LZWEncoder } from 'LZWEncoder';

const gifObj:iGIFObj = {
...
const canvasDataWOAlpha :iPixel[] = [];
const canvasSteam: number[] = [];
const pixelArray: number[] =[];
const colorMap : Array<Array<number>> = [];
...
getCanvasDataBtn.addEventListener('click', (event) => {
    ...
    gifObj.LSD.push(canvasData.width&0xff); gifObj.LSD.push(canvasData.width >> 8 &0xff) /* Width */
    gifObj.LSD.push(canvasData.height&0xff); gifObj.LSD.push(canvasData.height >> 8 &0xff) /* Height */
...
const setGCT = function(palette:number[]){
   let colors = [];
...
const setApplicationExtension = function(){
    ...
    gifObj.AppExten.push(0x0);gifObj.AppExten.push(0x0 >> 8);
    gifObj.AppExten.push(0x0);
}

const setImgDescriptor = function(width: number, height:number){
    gifObj.ImgDesc.push(0x2c); /* Image Seperator */
    gifObj.ImgDesc.push(0x0);gifObj.ImgDesc.push(0x0); /* Left */
    gifObj.ImgDesc.push(0x0);gifObj.ImgDesc.push(0x0); /* Top */
    gifObj.ImgDesc.push(width&0xff); gifObj.ImgDesc.push(width >> 8 &0xff) /* Width */
    gifObj.ImgDesc.push(height&0xff); gifObj.ImgDesc.push(height >> 8 &0xff) /* Height */
    gifObj.ImgDesc.push(0x0); /* Packed Field */
}
const getImgData = function(){

}
const setImgData = function(data: number[]) {
    gifObj.ImgData.push();
}
